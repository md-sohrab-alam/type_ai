...
class KeyboardViewModel @Inject constructor(
    ...
    private suspend fun loadSuggestions(context: String) {
        try {
            ...
            textSuggestionRepository.getSuggestionsByContext(context, limit)
                .catch { e -> _error.value = e.message }
                .collect { suggestionList ->
                    val suggestionTexts = suggestionList.map { it.text }
                    _suggestions.value = suggestionTexts
                }
            
            // Get AI-generated suggestions
            val aiSuggestions = grammarEngine.getSuggestions(_currentText.value, context, limit)
